 ...
 * classificação → salvar snapshot → publicar evento → métricas.
 *
...
public class GpsPositionIngestor {
    ...
    private final IngestionMetricsPort metrics;
    ...
    public IngestionResult ingest(List<VehiclePosition> positions) {
        ...
        for (VehiclePosition position : positions) {
            ...
            metrics.recordClassification(classification.status());
            metrics.recordPositionAge(Duration.between(position.positionTimestamp(), now));
            changed++;
        }
        IngestionResult result = new IngestionResult(positions.size(), duplicated, unchanged, changed);
